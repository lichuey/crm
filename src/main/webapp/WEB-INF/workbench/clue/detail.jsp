	...
	<body>
		...
		<div style="position: relative; left: 40px; top: -30px;">
			...
			<div style="position: relative; height: 50px; width: 500px;  top: -72px; left: 700px;">
				<button type="button" class="btn btn-default" onclick="window.location.href='toView/workbench/clue/convert?id=${id}'"><span class="glyphicon glyphicon-retweet"></span> 转换</button>
		...
		<script>
			...
			$("#name").keypress(function (event) {
				if (event.keyCode == 13) {
					$.get("workbench/clue/selectOtherActivity", {
						//id:clueId
						"id": "${id}",
						"name": $(this).val()
					}, function (data) {
						...
						for (var i = 0; i < activityList.length; i++) {
							...
							$("#showActivity").append("<tr>\n" +
									"\t\t\t\t\t\t\t\t\t<td><input value='"+ activity.id +"' type=\"checkbox\" class='son' onclick='abc()'/></td>\n" +
									"\t\t\t\t\t\t\t\t\t<td>" + activity.name + "</td>\n" +
									"\t\t\t\t\t\t\t\t\t<td>" + activity.startDate + "</td>\n" +
									"\t\t\t\t\t\t\t\t\t<td>" + activity.endDate + "</td>\n" +
									"\t\t\t\t\t\t\t\t\t<td>" + activity.owner + "</td>\n" +
